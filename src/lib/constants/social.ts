 ...
 */
export function buildXShareUrl({ url, text = DEFAULT_SHARE_TEXT, hashtags = DEFAULT_SHARE_HASHTAGS }: ShareUrlOptions): string {
 ...
 */
export function buildBlueskyShareUrl({ url, text = DEFAULT_SHARE_TEXT, hashtags = DEFAULT_SHARE_HASHTAGS }: ShareUrlOptions): string {
	const tagsStr = hashtags && hashtags.length > 0 ? ' ' + hashtags.map(cleanHashtag).filter(Boolean).map((t) => `#${t}`).join(' ') : '';
 ...
 */
export function buildThreadsShareUrl({ url, text = DEFAULT_SHARE_TEXT, hashtags = DEFAULT_SHARE_HASHTAGS }: ShareUrlOptions): string {
	const tagsStr = hashtags && hashtags.length > 0 ? ' ' + hashtags.map(cleanHashtag).filter(Boolean).map((t) => `#${t}`).join(' ') : '';
	const combined = `${text}${tagsStr} ${url}`.trim();
