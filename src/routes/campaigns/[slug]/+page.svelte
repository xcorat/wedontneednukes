...
<svelte:head>
	<title>{campaign.title} · Campaigns · We Don't Need Nukes</title>
	{#if campaign.summary}
	...
	<div class="w-full max-w-3xl">
		...
		<article class="border-2 border-border bg-surface p-6 sm:p-8 rounded-theme shadow-theme-md flex flex-col gap-6">
			<header class="flex flex-col gap-3 border-b-2 border-border/20 pb-5">
				...
				{#if campaign.website || campaign.founded || campaign.headquarters}
					<div class="mt-2 flex flex-wrap items-center gap-3 border-t border-border/20 pt-3">
						...
						{#if campaign.founded}
							<span class="inline-flex items-center gap-1 text-xs text-muted-foreground font-mono">
								<span class="font-bold text-foreground">Initiated:</span> {campaign.founded}
							</span>
						{/if}
						{#if campaign.headquarters}
							<span class="inline-flex items-center gap-1 text-xs text-muted-foreground font-mono">
								<span class="font-bold text-foreground">Base:</span> {campaign.headquarters}
							</span>
						{/if}
					</div>
				{/if}
